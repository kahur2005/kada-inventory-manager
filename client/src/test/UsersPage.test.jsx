...
function mockGet({ users, warehouses = [], stores = [] }) {
  apiClient.get = vi.fn((url) => {
    if (url === '/users') return Promise.resolve({ data: { users, total: users.length, page: 1, limit: 10 } });
...
describe('UsersPage', () => {
  ...
  test('lists users returned by the API', async () => {
    mockGet({ users: [{ id: '1', name: 'Ana', email: 'ana@example.com', role: 'unassigned', warehouse: null, store: null }] });
  ...
  test('changing role to warehouse_admin requires picking a warehouse before Save is enabled', async () => {
    mockGet({
      users: [{ id: '1', name: 'Ana', email: 'ana@example.com', role: 'unassigned', warehouse: null, store: null }],
    ...
    fireEvent.change(screen.getByLabelText(/role for ana/i), { target: { value: 'warehouse_admin' } });
    const saveButton = screen.getByRole('button', { name: /save/i });
    expect(saveButton).toBeDisabled();

    fireEvent.change(screen.getByLabelText(/warehouse for ana/i), { target: { value: 'wh1' } });
    expect(saveButton).not.toBeDisabled();

    fireEvent.click(saveButton);
    await waitFor(() => expect(apiClient.patch).toHaveBeenCalledWith('/users/1/role', { role: 'warehouse_admin', warehouse: 'wh1' }));
  });

  test('shows an error alert instead of silently failing when the role update is rejected', async () => {
    mockGet({ users: [{ id: '1', name: 'Ana', email: 'ana@example.com', role: 'unassigned', warehouse: null, store: null }] });
    apiClient.patch = vi.fn().mockRejectedValue({ response: { data: { message: 'boom' } } });
    ...
    await waitFor(() => expect(Swal.fire).toHaveBeenCalledWith(expect.objectContaining({ icon: 'error', text: 'boom' })));
  });
  ...
  test('shows store location info for a store_admin user', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Store One — 123 Main St')).toBeInTheDocument());
  });
